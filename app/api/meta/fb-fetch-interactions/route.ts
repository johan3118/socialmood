...
import { Interacciones } from "../../../../types/index";
import {
  obtenerCuentasRedesSociales,
  obtenerSoloReglasDeCuentas,
} from "../../../actions/(socialmood)/get-plans.actions";
...
): Promise<FacebookPageDetails | null> => {
  try {
    const response = await fetch(
      `${GRAPH_API_URL}/me?fields=id,name&access_token=${PAGE_ACCESS_TOKEN}`
    );
    if (!response.ok) {
      console.error(
        `Error fetching page details: ${response.status} - ${response.statusText}`
      );
      return null;
    }
...
const fetchPagePosts = async (
  ...
  try {
    const response = await fetch(
      `${GRAPH_API_URL}/${pageId}/feed?access_token=${PAGE_ACCESS_TOKEN}`
    );
    if (!response.ok) {
      console.error(
        `Error fetching page posts: ${response.status} - ${response.statusText}`
      );
      return [];
    }
    const data = await response.json();
...
const fetchAllPostCommentsBatch = async (
  ...
  for (const postId of postIds) {
    let url = `${GRAPH_API_URL}/${postId}/comments?order=reverse_chronological&since=${date}&access_token=${PAGE_ACCESS_TOKEN}`;
    const response = await fetch(url);

    if (!response.ok) {
      console.error(
        `Error fetching comments for post ${postId}: ${response.status} - ${response.statusText}`
      );
      break;
    }
    ...
    if (data.error) {
      console.error(
        `Error in response for comments on post ${postId}:`,
        data.error
      );
      break;
    }
...
const obtenerUltimaFechaRecepcion = async (
  cuentaId: string
): Promise<string> => {
...
async function sendInteraction(message: string) {
  try {
    const response = await fetch(
      "https://social-mood-dun.vercel.app/api/interactions/catalog",
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      throw new Error(
        `Error sending interaction: ${response.status} - ${response.statusText}`
      );
    }
  ...
  try {
    ...
    for (const cuenta of cuentas) {
      ...
      if (!pageDetails || !pageDetails.id) {
        console.error(
          `Unable to retrieve Page details for account ${cuenta.usuario_cuenta}`
        );
        continue;
      }
    ...
    return NextResponse.json(
      { error: "Failed to fetch or save posts and comments" },
      { status: 500 }
    );
  }
}