...
export default function Sidebar() {
  ...
  const [unansweredCounts, setUnansweredCounts] = useState<
      ...
      setUnansweredCounts((prevCounts) => {
        ...
        Object.keys(updatedCounts).forEach((key) => {
          if (updatedCounts[key] !== prevCounts[key]) {
            ...
            setTimeout(
              () => setAnimateItems((prev) => ({ ...prev, [key]: false })),
              1000
  ...
  return (
    <div className="w-64 h-screen bg-[#241F2C] text-white p-6 flex flex-col">
        ...
        {menuItems.map((item) => (
          <div key={item.id} className="mb-2">
            <button
              ...
              onClick={() =>
                item.subItems ? toggleExpand(item.id) : handleRoute(item.route)
              }
            >
      ...
      <button
        className="mt-auto p-2 rounded hover:bg-gray-800 flex items-center"
