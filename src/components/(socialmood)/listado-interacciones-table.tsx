...
const ListadoInteraccionesTable: React.FC = () => {
  const t = useTranslations("interactionsTable");
      ...
      <div className="container mx-auto p-6 ">
        <div className="flex justify-between mb-6">
          ...
          <div className="flex space-x-4">
            ...
            >
              <img src="/refresh.svg" alt={t("refresh")} className=" w-6 h-6" />
            </button>
          </div>
        </div>
          ...
          <table className="min-w-full table-auto">
            <thead>
              <tr className="text-[14px] md:text-[16px] lg:text-[18px]">
                <th className="py-3 md:py-4 px-2 md:px-6 lg:px-10 text-left">
                  {t("profile")}
                </th>
                <th className="py-3 md:py-4 px-2 md:px-6 lg:px-10 text-left">
                  {t("message")}
                ...
                <th className="hidden sm:table-cell py-3 md:py-4 px-2 md:px-6 lg:px-10 text-left ">
                  {t("category")}
                </th>
                <th className="hidden sm:table-cell py-3 md:py-4 px-2 md:px-6 lg:px-10 text-left">
                  {t("subcategory")}
                ...
                Interacciones.map((interacciones, index) => (
                  <tr key={index}>
                    <td className="py-3 md:py-4 px-4 md:px-8">
                      <div className="flex justify-center items-center space-x-2 w-full">
                            ...
                            buttonVariants({
                              variant:
                                interacciones.perfil.red_social === "Instagram"
                                  ? "orange"
                                  : interacciones.perfil.red_social ===
                                    "Facebook"
                                  ? "blue"
                                  : "default",
                              size: "smBold",
                        ...
                        >
                          <img
                            src={
                              socialIconMap[interacciones.perfil.red_social] ||
                              "/default.svg"
                            }
                    ...
                    <td className="py-2 md:py-3 px-2 md:px-4 lg:px-8">
                      {interacciones.mensaje}
                    </td>
                    <td className="py-2 md:py-3 px-2 md:px-4 lg:px-8">
                      {interacciones.emisor}
                    </td>
                    <td className="hidden sm:table-cell py-2 md:py-3 px-2 md:px-4 lg:px-8">
                      <div className="flex items-center space-x-2">
                        <span
                          className={cn(
                            buttonVariants({ variant: "angry", size: "smBold" })
                          )}
                          style={{ width: "100%", justifyContent: "center" }}
                        >
                          <img
                            src={
                              emojimap[interacciones.categoria] ||
                              "/default.svg"
                            }
                            alt="Emoji Icon"
                    ...
                    <td className="hidden sm:table-cell py-3 px-4 md:px-8 font-bold text-left ">
                      <div className="flex items-center justify-center space-x-2">
                        ...
                        >
                          <img
                            src={
                              emojimap[interacciones.subcategoria] ||
                              "/default.svg"
                            }
                            alt="Emoji Icon"
                            className="w-6 md:w-8 h-6 md:h-8 ml-2"
