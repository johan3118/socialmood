...
const InteraccionesDashboard: React.FC<InteraccionesDashboardProps> = ({
  filter = {},
}) => {
    ...
    <div className="w-full bg-gradient-to-b from-white/20 via-white/10 to-white/5 text-white border border-white/30 rounded-[28px] p-10 h-[300px]">
      <div className="grid grid-cols-1 gap-4">
        {interacciones.slice(0, 3).map((interaccion, index) => (
          <div
            key={index}
            className="flex items-center space-x-4 rounded-lg mb-4"
          >
            <span
              className={cn(
                buttonVariants({
            ...
            >
              <img
                src={
                  socialIconMap[interaccion.perfil.red_social] || "/default.svg"
                }
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center">
                <p className="font-medium text-sm text-gray-300 mr-4">
                  {interaccion.fecha}
                </p>
